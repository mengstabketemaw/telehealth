class Config {
  ...
  BASE = "https://telehealth-oauth-server.herokuapp.com" //`${this.SCHEME}://${this.HOST}:${this.PORT}`;
  VIDEOSERVER = `${this.SCHEME}://${this.HOST}:8081`
      ...
      if (error.response) {
        ...
        if (error.response.status) {
          message =
            ...
            "The server replayed with error code"
        }
        ...
        message =
          error.request || "Oops... There is no response from the server"
      } else {
