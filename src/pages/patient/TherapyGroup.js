...
const TherapyGroup = () => {
  ...
  useEffect(() => {
    ...
    VideoClient.get(
      VideoClient.PATIENT_THERAPY_GROUPS + token.username,
      success,
      error
    )
  }, [])
  ...
  const handleDelete = () => {
    ...
    const error = (message) => {
      setSnackbar({
        open: true,
        children:
          "Couldn't cancel reservation for the therapy group: " + message,
        severity: "error",
      })
      ...
      renderCell: (props) => {
        return (
          <Countdown date={props.value + ".000Z"}>
            {new Date(props.value + ".000Z").getTime() < Date.now + 1800000 ? (
              <p>Closed</p>
            ) : (
  ...
  return (
    <>
      ...
      {openDialog && (
        <Dialog fullWidth maxWidth={"100%"} open>
      ...
      {deleteDialog.open && (
        <Dialog onClose={() => setDeleteDialog(false)} open>
          <DialogTitle>Cancel Reservation for the Therapygroup</DialogTitle>
          <DialogContent>
            <DialogContentText>
              Are you sure you want to Cancel the appointment for the therapy
              group {deleteDialog.row?.therapy}
            </DialogContentText>
          </DialogContent>
          <DialogActions>
