...
const Doctors = () => {
  ...
  useEffect(() => {
    axios
      .get(Config.ADMIN_URL + "/doctors", Config.getAuthHeaders())
      .then(({ data }) => {
  ...
  const handleAccountStatus = (row) => {
    ...
    axios
      .put(
        Config.ADMIN_URL + `/user/${row.id}?disabled=${!row.disabled}`,
        null,
        Config.getAuthHeaders()
      )
      .then(() => {
        const newDate = data.row.map((e) => {
          if (e.id === row.id) return { ...e, disabled: !row.disabled }
          return e
        })
        setData({ loading: false, row: newDate })
        setSnackbar({
          open: true,
          children: `Account has been ${
  ...
  const handleDownloadApi = async (id) => {
    setSnackbar({
      open: true,
      children:
        "File is being download please wait, we will notify you when it is ready!",
      severity: "info",
    })
    try {
      // It doesn't matter whether this api responds with the Content-Disposition header or not
      const response = await axios.get(`${Config.ADMIN_URL}/doctor/${id}`, {
      ...
      setSnackbar({
        open: true,
        children: "couldn't download the file: " + e.message,
        severity: "error",
      })
    }
  }
  ...
  const column = [
      ...
      renderCell: ({ value }) => {
        return (
          <Avatar
            ...
            src={`${Config.USER_URL}/avatar/${value}`}
          />
        )
      },
    },
    ...
    },
    {
      ...
      renderCell: ({ value }) => {
        return value ? (
          <Chip icon={<DisabledByDefault />} color={"error"} label="Disabled" />
        ) : (
          <Chip
    ...
    },
    {
      ...
      getActions: ({ row }) => {
        return [
          <GridActionsCellItem label="View Profile" showInMenu />,
          <GridActionsCellItem
            label={row.disabled ? "Enable Account" : "Disable Account"}
            showInMenu
            icon={
              row.disabled ? <Done color="success" /> : <DisabledByDefault />
            }
