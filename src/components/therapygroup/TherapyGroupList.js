...
const TherapyGroupList = ({ setData, handleClose }) => {
  ...
  useEffect(() => {
    ...
    const error = (message) => {
      setSnackbar({
        open: true,
        children: "Could't fetch data from the server: " + message,
        severity: "error",
      })
      handleClose()
    }
  ...
  const handleBookAppointment = (row) => {
    ...
    const success = () => {
      ...
      setSnackbar({
        open: true,
        children:
          "Appointment has been Booked, you can join the conference by clicking on the join, when the time come.",
        severity: "success",
      })
    }
    ...
    const error = (message) =>
      setSnackbar({
        open: true,
        children: "Could't book reservation: " + message,
        severity: "error",
      })

    VideoClient.get(
      VideoClient.JOIN_THERAPY_GROUP + row.id + "/" + token.username,
      success,
      error
    )
  }
  const conditionForBooking = (id, date) => {
    ...
    if (
      new Date(date + ".000Z").getTime() > Date.now() &&
      current.patients.every((pat) => pat !== token.username) &&
      current.patients.length < current.maxPatientNumber
    )
      return false
    return true
  }
